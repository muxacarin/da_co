...
public class DavinchiCodeHelper {
	...
	private GameHandler gamehandler;	//게임판 : ui제어
//	private DavinchiCodePlayer[] pa = new DavinchiCodePlayer[4];
	private ArrayList<DavinchiCodePlayer> pAL = new ArrayList<DavinchiCodePlayer>(); //플레이어 지정용 어레이리스트
	private int number_of_gamer;//게임하는 사람 수
		...
		if (this.number_of_gamer == 4){ //게이머수 4명 : 초기 패 3장
			...
			for (int i = 0; i < 4; i++){//각 게이머 마다
				for (int j = 1; j<=3;){//패 3장이 될동안
					if(this.addcard(i, (this.ran.nextBoolean()) ? Dacolor.BLACK : Dacolor.WHITE)) j++;//카드를 뽑아준다
				}
			}
	...
	public void takecard(int peopleindex, Dacolor color){//카드 가져오기 메소드
		...
		while(!this.addcard(peopleindex%this.number_of_gamer, color));//카드를 얻어온다
		this.turnindex = Turn.SELECT;//턴의 단계 : 카드 맞추기
		this.gamehandler.update();//ui 업데이트
		
	}
	
	public void assertcard(int player, int cardpos, Dacolor color, int number){//카드 맞추기 메소드
		int playerindex = (player + this.turn)%this.number_of_gamer;//지목한 카드의 주인의 인덱스 설정
	...
	private boolean addcard (int playerindex, Dacolor color){//카드 가져오기 구현
		...
		case BLACK://검정이라면
			if (this.deck_black[cardnumber]){//검정 랜덤카드를 줄 수있다면
				this.pAL.get(playerindex).cardset(Dacolor.BLACK, cardnumber);//카드 추가
				this.deck_black[cardnumber] = false;//덱에서 삭제
				this.setRemainder_black(this.getRemainder_black() - 1);//카드 수 1 감소
	...
	private void nextturn() {//턴 넘기기 구현
		do{
			this.turn++;//턴을 증가시킨다
		} while(!pAL.get(this.turn%this.number_of_gamer).Islive());//다음사람이 죽었다면 계속 반복
